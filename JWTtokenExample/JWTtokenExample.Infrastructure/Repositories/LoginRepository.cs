...
namespace JWTtokenExample.Infrastructure.Repositories
{
	...
	public class LoginRepository<TUser> : ILoginRepository<TUser> where TUser : ApplicationUser
	{
		...
		private JwtSecurityToken CreateToken(List<Claim> authClaims)
		{
			...
			var token = new JwtSecurityToken(issuer: _configuration["JWT:ValidIssuer"], audience: _configuration["JWT:ValidAudience"], expires: DateTime.Now.AddMinutes(TokenValidityInHours), claims: authClaims, signingCredentials: new SigningCredentials(authSigningKey, SecurityAlgorithms.HmacSha256));
			return token;
		}
